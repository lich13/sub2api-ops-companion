...
从本仓库 Releases 下载 `Sub2Ops_0.1.0_aarch64.dmg`，校验 `SHA256SUMS` 后将 Sub2Ops 拖入 Applications。首版使用本机签名，未经 Apple 公证；检查更新只打开发布页，不自动安装。
...
- 总览：按真实使用日志的分组显示最近成功账号、模型及时间。
- 账号：分组、平台、类型、状态筛选；调度开关只改变允许调度，不清除限流、冷却或认证错误。
...
左键菜单栏图标打开快捷面板，星标分组排在前面；图钉控制失焦后是否关闭。右键打开主窗口、检查更新或退出。关闭主窗口继续驻留菜单栏，开机启动默认关闭。
